...
class Handlers:
    ...
    @staticmethod
    def method_not_allowed(request:Request,*args,**kwargs)->Response:
        response={
            "message": f"Requested method:{request.method} is not allowed for {request.path}"
        }
        return Response(json_body=response,status=HttpStatus.METHOD_NOT_ALLOWED)
